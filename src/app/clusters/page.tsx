...
import { mockClusters, mockItems } from "@/config/mock-data"
...
function ClusterRow({ cluster }: { cluster: typeof mockClusters[number] }) {
  const [expanded, setExpanded] = useState(false)

  const primaryItem  = mockItems.find(i => i.id === cluster.primaryItemId)
  const relatedItems = mockItems.filter(i => cluster.relatedItemIds.includes(i.id))
    ...
    <div className="border border-border rounded-lg overflow-hidden bg-card">
      ...
      >
        ...
        <div className="flex-1 min-w-0 space-y-0.5">
          ...
          {/* Core signal — one-line from primary item summary */}
          {primaryItem && (
          ...
          {/* Meta row: reports · first seen · latest · momentum · tier chips */}
        ...
        <div className="border-t border-border">
          ...
          {uncoveredCount > 0 && (
            <div className="px-5 py-2.5 text-center border-t border-border bg-surface">
              <span className="text-xs text-muted-foreground">另有 {uncoveredCount} 篇报道未录入 mock 数据</span>
            </div>
          )}
        </div>
      )}
...
export default function ClustersPage() {
  const sorted = [...mockClusters].sort((a, b) => b.momentum - a.momentum)
      ...
      <div className="p-6 md:p-8 max-w-[1080px]">
        ...
        <div className="mb-6">
          ...
          <p className="page-subtitle mt-1.5">
            {mockClusters.length} 个活跃事件 · 按势头排序
          </p>
        </div>
        ...
        <div className="grid grid-cols-4 gap-3 mb-6">
          {[
            { label: '活跃事件',   value: mockClusters.length },
            { label: '总关联报道', value: mockClusters.reduce((s, c) => s + c.sourceCount, 0) },
            { label: '高势头 ≥80', value: mockClusters.filter(c => c.momentum >= 80).length },
            { label: '今日新增',   value: mockClusters.filter(c => {
