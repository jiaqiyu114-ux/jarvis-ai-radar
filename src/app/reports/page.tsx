...
import { mockReports, mockItems, mockTopics } from "@/config/mock-data"
import { cn } from "@/lib/utils"

/* ─── Build text helpers ─── */
function buildMarkdown(report: typeof mockReports[number]): string {
  return [
...
function buildWechatDraft(report: typeof mockReports[number]): string {
  return [
    `【AI 日报 ${report.date}】`,
    '',
    '今日速览：',
...
function buildXhsTopics(report: typeof mockReports[number]): string {
  return [
...
export default function ReportsPage() {
  ...
  const worthWriting = mockTopics.filter(t => t.status === 'worth_writing').length
  const todayHighItems = mockItems
